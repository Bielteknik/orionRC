...
apiRouter.post('/sensors', async (req: Request, res: Response) => {
    ...
    await db.run('INSERT INTO sensors (id, name, station_id, type, status, is_active, battery, last_update, value, interface, config, parser_config, read_frequency) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', newId, name, stationId, type, isActive ? 'Aktif' : 'Pasif', isActive, 100, new Date().toISOString(), '{}', interfaceType, interfaceConfig, parserConfig, readFrequency);
...
apiRouter.put('/sensors/:id', async (req: Request, res: Response) => {
    const { id } = req.params;
    const { name, stationId, type, isActive, interfaceType, interfaceConfig, parserConfig, readFrequency } = req.body;
     await db.run('UPDATE sensors SET name = ?, station_id = ?, type = ?, status = ?, is_active = ?, last_update = ?, interface = ?, config = ?, parser_config = ?, read_frequency = ? WHERE id = ?', name, stationId, type, isActive ? 'Aktif' : 'Pasif', isActive, new Date().toISOString(), interfaceType, interfaceConfig, parserConfig, readFrequency, id);
