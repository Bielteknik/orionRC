...
export default class OpenWeatherDriver implements ISensorDriver {
     ...
     */
    public async read(config: { apikey: string; lat: number; lon: number; }, verbose: boolean = true): Promise<Record<string, any> | null> {
        const { apikey, lat, lon } = config;

        if (!apikey || lat === undefined || lon === undefined) {
            if (verbose) console.error("     -> HATA (OpenWeather): API anahtarı veya koordinatlar eksik. Bu yapılandırma sunucu tarafından sağlanmalıdır.");
            return null;
        }

        const url = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apikey}&units=metric`;
        
        if (verbose) console.log(`     -> OpenWeather API'den veri alınıyor...`);
        ...
        try {
            ...
            if (response.status === 200 && response.data && response.data.main) {
                ...
                if (verbose) console.error(`     -> HATA (OpenWeather): API'den geçersiz yanıt alındı. Durum: ${response.status}`);
                return null;
            }
